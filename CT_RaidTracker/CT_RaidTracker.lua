...
-- >>>>>> add by junyuli <<<<<<
function toexcel_date(dDate) 
	local _, _, t_mon, t_day, t_year, t_hr, t_min, t_sec = string.find(dDate, "(%d+)/(%d+)/(%d+) (%d+):(%d+):(%d+)");
	
	return t_year .. "/" .. t_mon .. "/" .. t_day .. " " .. t_hr .. ":" .. t_min .. ":" .. t_sec;
end

function CT_RaidTrackerGenerateRaidDkpLink(id)
	local player_records = {};
	local player_records_text = "";
	local raid_start_time = "";
	local raid_end_time = "";
	local default_join_score = 5;
	local default_full_join_score = 10;

	if( CT_RaidTracker_RaidLog[id]["key"] ) then 
		local _, _, t_mon, t_day, t_year, t_hr, t_min, t_sec = string.find(CT_RaidTracker_RaidLog[id]["key"], "(%d+)/(%d+)/(%d+) (%d+):(%d+):(%d+)");
		
		raid_start_time	= t_mon .. "/" .. t_day .. "/" .. t_year .. " " .. "20:00:00";
		raid_end_time 	= t_mon .. "/" .. t_day .. "/" .. t_year .. " " .. "23:00:00";
	end 

	-- 遍历玩家信息, 形成[玩家名称, 开始时间, 结束时间, 集合分, 全程分, 装备均分]的数据结构
	-- { "class" = "", "join_time" = "", "leave_time" = "", "join_score" = "", "full_join_score" = "", "equip_score" = "" };
	if(CT_RaidTracker_RaidLog[id]["PlayerInfos"]) then

		for infos_key, infos_val in CT_RaidTracker_RaidLog[id]["PlayerInfos"] do
			player_records[infos_key] = {};
			player_records[infos_key]["join_score"] = 0
			player_records[infos_key]["full_join_score"] = default_full_join_score;
			player_records[infos_key]["equip_score"] = 0;
			

			-- 遍历职业、等级、种族信息
			for infos_detail_key, infos_detail_val in CT_RaidTracker_RaidLog[id]["PlayerInfos"][infos_key] do
				player_records[infos_key][infos_detail_key] = infos_detail_val;
			end
			
			-- 遍历加入时间, 取最早加入时间
			for join_key, join_val in pairs(CT_RaidTracker_RaidLog[id]["Join"]) do

				if( infos_key == join_val["player"] ) then
					
					if( not player_records[join_val["player"]]["join_time"] 
					     or CT_RaidTracker_GetTime(player_records[join_val["player"]]["join_time"]) > CT_RaidTracker_GetTime(join_val["time"]) ) then
						player_records[join_val["player"]]["join_time"] = join_val["time"];
					end
			...
			-- 遍历离开时间, 取最晚离开时间
			for leave_key, leave_val in pairs(CT_RaidTracker_RaidLog[id]["Leave"]) do

				if( infos_key == leave_val["player"] ) then
					
					if( not player_records[leave_val["player"]]["leave_time"] 
					     or CT_RaidTracker_GetTime(player_records[leave_val["player"]]["leave_time"]) < CT_RaidTracker_GetTime(leave_val["time"]) ) then
						player_records[leave_val["player"]]["leave_time"] = leave_val["time"];	
					end	
			...
			--计算这个人的集合分
			if( CT_RaidTracker_GetTime(player_records[infos_key]["join_time"]) <= CT_RaidTracker_GetTime(raid_start_time) )  then
				player_records[infos_key]["join_score"] = default_join_score;
			else
				player_records[infos_key]["join_score"] = 0;
			end


			--计算这个人的全程分, 比较加入时间和BOSS击杀时间和离队时间
			--[[
			if( CT_RaidTracker_GetTime(player_records[infos_key]["leave_time"]) >= CT_RaidTracker_GetTime(raid_end_time) )  then
				player_records[infos_key]["full_join_score"] = default_full_join_score;
			else
				player_records[infos_key]["full_join_score"] = 0;
			end
			]]
			for kill_key, kill_val in CT_RaidTracker_RaidLog[id]["BossKills"] do
				if( not ( CT_RaidTracker_GetTime(player_records[infos_key]["leave_time"]) > CT_RaidTracker_GetTime(kill_val["time"]) 
							and
						  CT_RaidTracker_GetTime(player_records[infos_key]["join_time"]) < CT_RaidTracker_GetTime(kill_val["time"]) 
					) 
				) then
					player_records[infos_key]["full_join_score"] = 0;
					break;
			...
			--计算这个人的装备均分
			for loot_key, loot_val in CT_RaidTracker_RaidLog[id]["Loot"] do
				-- 如果这件物品记录了分数, 否则不予记录
				if( loot_val["costs"] ) then
					
					--判断该玩家是否参与分配装备, 若有则累加分数
					for attend_key, attend_val in loot_val["attendees"] do
						
						if infos_key == attend_val then
							player_records[infos_key]["equip_score"] = player_records[infos_key]["equip_score"] + tonumber(loot_val["costs"]);
						end
			...
			-- 计算装备平均得分,向下取整
			player_records[infos_key]["equip_score"] = math.floor( player_records[infos_key]["equip_score"] / 40 );

		end
	end
	
	for player_key, player_val in player_records do 
		player_records_text = player_records_text .. player_key .. "," .. toexcel_date(player_val["join_time"]) .. "," .. toexcel_date(player_val["leave_time"]) .. "," .. player_val["join_score"] .. "," .. player_val["full_join_score"] .. "," .. player_val["equip_score"] .. "|$|"
	end
	
	CT_RaidTrackerShowDkpLink(player_records_text);
end
-- >>>>>> end add by junyuli  <<<<<<
...
function CT_RaidTracker_RaidsRightClickMenu_Initialize(level)
	...
	if (level == 1) then
		...
		-->>> start, junyuli, add <<<--
		info = {};
		info.text = "导出自动计算分数文本";
		info.value = { ["raidid"] = raidid};
		info.func = function()
			HideDropDownMenu(1);
			CT_RaidTrackerGenerateRaidDkpLink(this.value["raidid"]);
		end;
		UIDropDownMenu_AddButton(info, level);
		-->>> end, junyuli, add <<<--
