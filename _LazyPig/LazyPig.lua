...
BINDING_HEADER_LP_HEADER = "_懒猪";
BINDING_NAME_LOGOUT = "注销";
BINDING_NAME_UNSTUCK = "松开";
BINDING_NAME_RELOAD = "重载UI";
BINDING_NAME_DUEL = "目标战歌敌对旗手/取消决斗";
BINDING_NAME_WSGDROP = "战歌快捷放旗/消除缓落术";
BINDING_NAME_MENU = "_懒猪 菜单";
...
function LazyPig_OnEvent(event)
	if (event == "ADDON_LOADED") and (arg1 == "_LazyPig") then
		...
		DEFAULT_CHAT_FRAME:AddMessage(LP_TITLE .. " 版本" .. LP_VERSION .. " by " .."|cffFF0066".. LP_AUTHOR .."|cffffffff".. " 加载, 使用".."|cff00eeee".." /lp".."|cffffffff 设置 by 60addons汉化")
	...
	elseif(event == "DUEL_REQUESTED") then
		...
		if LPCONFIG.DUEL and not IsShiftKeyDown() then --dnd_active and
			duel_active = nil
			CancelDuel()
			UIErrorsFrame:AddMessage(arg1.." - 决斗取消")
		end	
	...
	elseif(event == "START_LOOT_ROLL") then
		...
		if(curZone == "祖尔格拉布") then
			LazyPig_ZGRoll(arg1)
		elseif(curZone == "安其拉废墟") then
			LazyPig_AQRoll(arg1)
		end
	
	elseif(event == "CHAT_MSG_LOOT") then
		if (string.find(arg1 ,"你赢得了") or string.find(arg1 ,"你得到了")) and (string.find(arg1 ,"cffa335e") or string.find(arg1, "cff0070d") or string.find(arg1, "cffff840")) and not string.find(arg1 ,"宝石") and not string.find(arg1 ,"雕像") and not string.find(arg1 ,"碎片") then
			save_time = GetTime()
		end
	
	elseif(event == "UI_ERROR_MESSAGE") then
		if(string.find(arg1, "你正在") or string.find(arg1, "无法在")) and LPCONFIG.DISMOUNT then
			UIErrorsFrame:Clear()
			LazyPig_Dismount()
		end
	elseif (event == "UI_INFO_MESSAGE") then
		if string.find(arg1 ,"决斗取消") then
			duel_active = nil
		end	
	elseif (event == "CHAT_MSG_SYSTEM") then
		...
		elseif(string.find(arg1, string.sub(MARKED_AFK, 1, string.len(MARKED_AFK) -2))) then
			afk_active = true
			if LPCONFIG.EBG and not LazyPig_Raid() and not LazyPig_Dungeon() then UIErrorsFrame:AddMessage("自动加入战场 - 停止AFK") end
		
		elseif string.find(arg1, "There is no such command") and (GetTime() - save_check) < 1 then
			LPCONFIG.NOSAVE = GetRealmName()
			DEFAULT_CHAT_FRAME:AddMessage("LazyPig:"..RED.."自动保存禁用 - 命令不支持");		
	...
	elseif(event == "RESURRECT_REQUEST" and LPCONFIG.REZ) then
		UIErrorsFrame:AddMessage(arg1.." - 复活术")
...
function LazyPig_StaticPopup_OnShow()
	if this.which == "QUEST_ACCEPT" and LazyPig_BG() and LPCONFIG.SBG then
		UIErrorsFrame:Clear();
		UIErrorsFrame:AddMessage("任务成功封堵");
		this:Hide();
...
function MailtoCheck(msg) 
	if MailTo_Option then -- to avoid conflicts with mailto addon
		...
		if msg then DEFAULT_CHAT_FRAME:AddMessage("_LazyPig: 右键单击警告改进易分割/合并功能可以重写邮件插件的功能 !") end
	end
end
...
function AcceptGroupInvite()
	...
	UIErrorsFrame:AddMessage("组队自动接受");
end
...
function LazyPig_AutoSummon()
	...
	if LPCONFIG.SUMM then
		...
		if not player_summon_message and expireTime ~= 0 then
			...
			DEFAULT_CHAT_FRAME:AddMessage("LazyPig: 自动召唤 "..math.floor(expireTime).."秒", 1.0, 1.0, 0.0);
...
function Check_Bg_Status()
	...
	for i=1, MAX_BATTLEFIELD_QUEUES do
		...
		elseif((status == "active") and not (mapName == "东部王国") and not (mapName == "卡利姆多")) then
...
function LazyPig_AutoJoinBG(index, map_name)	
	...
	if LPCONFIG.EBG or keyenter then	
		...
		if not player_bg_message and expireTime > 3 and GetTime() > delayaction then
			...
			DEFAULT_CHAT_FRAME:AddMessage("LazyPig: 自动加入 ".. map_name.." 在 "..expireTime.."秒", 1.0, 1.0, 0.0)
...
function LazyPig_AutoLeaveBG()
	...
	if LPCONFIG.LBG or keyenter then
		...
		local winner_name = "联盟"
		if bg_winner ~= nil then	
			save_time = GetTime() + 15
			if bg_winner == 0 then winner_name = "部落" end
			UIErrorsFrame:Clear();
			UIErrorsFrame:AddMessage(winner_name.." 胜利了");
			LeaveBattlefield();
		end	
	end
end
...
function LazyPig_ZGRoll(id)
	...
	if LPCONFIG.ZG then	
		...
		if string.find(name ,"哈卡莱宝石") or string.find(name ,"硬币") then
			...
			DEFAULT_CHAT_FRAME:AddMessage("LazyPig: 自动 "..hex..RollReturn().." "..GetLootRollItemLink(id))
			return
		end	
	end	
end
...
function LazyPig_AQRoll(id)
	...
	if LPCONFIG.AQ then	
		local _, name, _, quality = GetLootRollItemInfo(id);
		if string.find(name, "雕像") or string.find(name, "甲虫") then
	...
	if bag and slot then
		...
		if bag and slot and not locked then	
			...
			if not(GreySell.bag == bag and GreySell.slot == slot) then
				DEFAULT_CHAT_FRAME:AddMessage("LazyPig: 销售 "..GetContainerItemLink(bag, slot))
				GreySell.bag = bag
				GreySell.slot = slot
			end	
		end
	elseif CanMerchantRepair() then
		...
		if rcost and rcost ~= 0 then
			if rcost > GetMoney() then 
				DEFAULT_CHAT_FRAME:AddMessage("LazyPig: 没有足够的钱来修理")
				return
			end
		...
		elseif GreySell.repair and	rcost == 0 then
			...
			DEFAULT_CHAT_FRAME:AddMessage("LazyPig: 修复所有物品 "..COLOR_GOLD..gold.."金 "..COLOR_SILVER..silver.."银 "..COLOR_COPPER..copper.."铜")
			GreySell.repair = nil
		end
	end	
end
...
function LazyPig_FixQuest(quest, annouce)
	...
	if UnitLevel("player") == 60 then	
		if string.find(quest, "自然屏障") then
			QuestRecord["details"] = "自然屏障 60"
		elseif string.find(quest, "峡谷之战") then
			QuestRecord["details"] = "峡谷之战 60"	
		elseif string.find(quest, "战争的动力") then
			QuestRecord["details"] = "战争的动力 60"	
		elseif string.find(quest, "资源竞赛") then
			QuestRecord["details"] = "资源竞赛 60"
		end
	end
...
function LazyPig_DropWSGFlag_NoggBuff()
	local counter = 0
	local tooltipfind1 = "战歌军旗"
	local tooltipfind2 = "You feel light"
	local tooltipfind3 = "缓落术"
...
function LazyPig_Raid()
	...
	if t =="熔火之心" or t =="黑翼之巢" or t =="祖尔格拉布" or t =="安其拉" or t =="奥妮克希亚的巢穴" or t =="安其拉废墟" or t =="安其拉神殿" or t =="纳克萨玛斯" or t =="黑石塔" then
		return true
	end
	return false
end
...
function LazyPig_Dungeon()
	local t = GetRealZoneText()
	if t =="厄运之槌" or t =="斯坦索姆"  or t =="通灵学院" or t =="黑石深渊" or t =="沉没的神庙" or t =="监狱" or t =="祖尔法拉克" or t =="血色修道院" or t =="诺莫瑞根" or t =="死亡矿井" or t=="黑暗深渊" or t=="哀嚎洞穴" or t=="剃刀高地" or t=="剃刀沼泽" or t=="怒焰裂谷" or t=="影牙城堡" then 
		return true
	end
	return false
end
...
function LazyPig_BG()
	local t = GetRealZoneText()
	if t == "阿拉希盆地" or t == "战歌峡谷" or t == "奥特兰克山谷" then
		return true
	end
	return false
end
...
function LazyPig_UseContainerItem(ParentID,ItemID)
		...
		elseif LPCONFIG.RIGHT and tradestatus and not IsShiftKeyDown() and not IsAltKeyDown() then
			if not LazyPig_ItemIsTradeable(ParentID,ItemID) then
				DEFAULT_CHAT_FRAME:AddMessage("LazyPig: 无法附加项目", 1, 0.5, 0);
				return
			end
			...
			for i = 1, GMAIL_NUMITEMBUTTONS, 1 do
				if ( not getglobal("GMailButton" .. i).item ) then
					...
					if ( GMail:ItemIsMailable(bag, item) ) then
						GMail:Print("GMail: 无法附加项目.", 1, 0.5, 0)
						return
					end
		...
		elseif LPCONFIG.RIGHT and auctionstatus and IsAltKeyDown() then
			if not LazyPig_ItemIsTradeable(ParentID,ItemID) then
				DEFAULT_CHAT_FRAME:AddMessage("LazyPig: 物品不是可交易的", 1, 0.5, 0);
				return
			end
...
function LazyPig_IsBearForm()
	...
	for i = 1 , max do
		...
		if(isActive and LazyPig_PlayerClass("德鲁伊", "player") and (name == "熊形态" or name == "巨熊形态")) then
			return true
		end
	end
	return false
...
function LazyPig_CheckSalvation()
	if(LPCONFIG.SALVA == 1 or LPCONFIG.SALVA == 2 and (LazyPig_IsShieldEquipped() and LazyPig_PlayerClass("战士", "player") or LazyPig_IsBearForm())) then
...
function LazyPig_ChatFrame_OnEvent(event)
	if event == "CHAT_MSG_LOOT" or event == "CHAT_MSG_MONEY" then
		local bijou = string.find(arg1 ,"宝石")
		local coin = string.find(arg1 ,"硬币")
		local scarab = string.find(arg1, "甲虫")
		local idol = string.find(arg1, "雕像")
		...
		local check_loot = LPCONFIG.SPAM_LOOT and (string.find(arg1 ,"9d9d9d") or string.find(arg1 ,"ffffff") or string.find(arg1 ,"你的战利品分享"))
		local check_money = LPCONFIG.SPAM_LOOT and string.find(arg1 ,"你的战利品分享")
	
		local check1 = string.find(arg1 ,"你")
		local check2 = string.find(arg1 ,"赢得了") or string.find(arg1 ,"得到了")
...
function LazyPig_Target_EFC()
	...
	if wsgefc then
		...
		if not UnitExists("target") then 
			UIErrorsFrame:AddMessage("超出范围 - 敌方旗手 - "..strupper(wsgefc)) 
		elseif strlower(GetUnitName("target")) == wsgefc then
			local class, classFileName = UnitClass("target")
			local color = RAID_CLASS_COLORS[classFileName]
			UIErrorsFrame:AddMessage(strupper(class.." - 敌方旗手 - "..wsgefc), color.r, color.g, color.b) 
		end
	end	
end

function LazyPig_Duel_EFC()
	if GetRealZoneText() == "战歌峡谷" then
		LazyPig_Target_EFC()
	else
		local duel = nil
		for i=1,STATICPOPUP_NUMDIALOGS do
