...
function WIM_Options_OnShow()
	...
	--[ Initialize Display Settings Frame 
		...
		WIM_OptionsDisplayWindowSizeTitle:SetText("窗口大小 (百分比)");
		WIM_OptionsDisplayWindowAlpha:SetValue(WIM_Data.windowAlpha * 100);
		WIM_OptionsDisplayWindowAlphaTitle:SetText("透明度 (百分比)");
...
function WIM_Options_ShowShortcutBarClicked()
	if(WIM_OptionsDisplayShowShortcutBar:GetChecked()) then
		...
		WIM_OptionsTabbedFrameWindowWindowHeightTitle:SetText("窗口高度 |cffffffff|r");
	else
...
function WIM_Options_AliasWindow_Click()
	...
	if(name == "") then
		WIM_Options_AliasWindow_Error:SetText("错误：无效名称!");
		return;
	end
	if(alias == "") then
		WIM_Options_AliasWindow_Error:SetText("错误：无效别名!");
		return;
	end
	if(WIM_Options_AliasWindow.theMode == "add" and WIM_Alias[name] ~= nil) then
		WIM_Options_AliasWindow_Error:SetText("错误：名称已被使用!");
		return;
	end
...
function WIM_Options_FilterWindow_Click()
	...
	if(tname == "") then
		WIM_Options_FilterWindow_Error:SetText("错误：无效的关键词/短语！");
		return;
	end
	if(WIM_Options_FilterWindow.theMode == "add" and WIM_Filters[name] ~= nil) then
		WIM_Options_FilterWindow_Error:SetText("错误：关键词/短语已被使用！");
		return;
	end
