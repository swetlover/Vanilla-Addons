...
---------------------------------------------------------------------------
	...
	local GainFadeAfflict = function(info)
		local gain = '(.+)获得了(.+)的效果。' 								local fgain = string.find(arg1, gain)
		local fade = '(.+)效果从(.+)身上消失。'							local ffade = string.find(arg1, fade)
		local afflict = '(.+)受到了(.+)效果的影响。' 					local fafflict = string.find(arg1, afflict)
		local rem = '(.+)的(.+)被移除了。'							local frem = string.find(arg1, rem)
