...
do
	...
	function CHAT_MSG_SPELL_FAILED_LOCALPLAYER()
		for action in string.gfind(arg1, '你%a+(.*)失败：.*') do
			casting[action] = nil
		end
	end
...
function CHAT_MSG_COMBAT_HOSTILE_DEATH()
	for unit in string.gfind(arg1, '(.+)死亡') do -- TODO does not work when xp is gained
		if IsPlayer(unit) then
			UnitDied(unit)
...
function CHAT_MSG_COMBAT_HONOR_GAIN()
	for unit in string.gfind(arg1, '(.+)死亡') do
		UnitDied(unit)
	end
end
...
function AUF.Debuff:Build()
	for i=1,16 do
		...
		if getglobal("pfUITargetDebuff1") then AUF.Debuff[i].Font:SetFont("Fonts\\ARIALN.TTF", AUF_settings.TextSize, "OUTLINE") end
...
function AUF.Buff:Build()
	for i=1,16 do
		...
		if getglobal("pfUITargetBuff1") then AUF.Buff[i].Font:SetFont("Fonts\\ARIALN.TTF", AUF_settings.TextSize, "OUTLINE") end
			...
			if not timer.DR and AUF.UnitName("target") == timer.UNIT then
				for i=1,16 do
					if UnitDebuffText("target",i) == timer.EFFECT and getglobal(AUF.DebuffAnchor..i) and not AUF.DoubleCheck[timer.EFFECT] then
						if timer.EFFECT == "破胆怒吼" then -- exception
						else
...
function AUF:BuildOptions()
	...
	AUF.Options.Headline:SetText("DebuffTimers 设置")
	...
	AUF.Options.DebuffFont:SetText("显示的Debuff:")
	...
	AUF.Options.SizeFont:SetText("计时器字体大小:")
	...
	AUF.Options.OkButton:SetText("保存")
...
function AUF:SaveOptions()
	...
	for i=1,16 do
		AUF.Debuff[i].Font:SetFont("Fonts\\ARIALN.TTF", AUF_settings.TextSize, "OUTLINE")
		if getglobal("pfUITargetDebuff1") then AUF.Debuff[i].Font:SetFont("IFonts\\ARIALN.TTF", AUF_settings.TextSize, "OUTLINE") end
		AUF.Buff[i].Font:SetFont("Fonts\\ARIALN.TTF", AUF_settings.TextSize, "OUTLINE")
		if getglobal("pfUITargetBuff1") then AUF.Buff[i].Font:SetFont("Fonts\\ARIALN.TTF", AUF_settings.TextSize, "OUTLINE") end
	end
	...
	DEFAULT_CHAT_FRAME:AddMessage("|cFF00FF00Debuff Timers:|r 设置保存.|r",1,1,1)
	AUF.Options:Hide()
end
...
function AUF:DatabasePreload()
	if CLASS == "MAGE" then
		...
		if rank == 3 then
			AUF_Debuff["MAGE"].EFFECT["痛苦诅咒"] = {
				ICON = "Spell_Fire_SoulBurn",
				DURATION = 30,
			}
			
			AUF_Debuff["MAGE"].SPELL["灼烧"] = {
