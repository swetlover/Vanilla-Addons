--设置宏命令为：
--/script local _G = getfenv(0);_G.FocusCore:SetFocus(UnitName("target"))
--'mousemover'也行，没有目标则清除
...
--FocusFrameHealthBarColor
RAID_CLASS_COLORS['SHAMAN'].r = 0 / 255
RAID_CLASS_COLORS['SHAMAN'].g = 112 / 255
RAID_CLASS_COLORS['SHAMAN'].b = 222 / 255

local FocusFrameHealthBarColor = function(u)
...
local function OnFocusSat(event, unit)
	local fontName, fontSize, fontFlags = TargetName:GetFont()
	
	FocusName:SetText(UnitName(unit))
	FocusName:SetFont(fontName, fontSize, 'OUTLINE')
	FocusFrame:SetScale(FocusFrameDB.scale or 1)

	FocusFrame:SetScript("OnUpdate", FocusFrame_CastingBarOnUpdate)
	FocusFrame:Show()
	--血条职业颜色
	FocusFrameHealthBar:SetStatusBarColor(FocusFrameHealthBarColor'target')
end
...
FocusFrame.cast.text:SetFont(STANDARD_TEXT_FONT, 10,  'OUTLINE')
...
FocusFrame.cast.timer:SetFont(STANDARD_TEXT_FONT, 10,  'OUTLINE')
...
SlashCmdList.FOCUSOPTIONS = function(msg)
	...
	elseif cmd == "reset" then
		...
		print("    scale 1.0 - |cff00FF7F 改变框架大小 (0.2 - 2.0)")
		print("    lock - |cff00FF7F 拖动框架")
		print("    nohide - |cff00FF7F 在加载屏幕/施放灵魂下切换框架自动隐藏")
		print("    fade - |cff00FF7F 当焦点没有被更新时（10秒）淡出")
		print("    strictaura - |cff00FF7F 切换光环/施放优化")
		print("    noplates - |cff00FF7F 启用姓名板扫描")
		print("    statustext - |cff00FF7F 显示血量/蓝量数值")
		print("    reset - |cff00FF7F 重置为默认设置")
	end
end
