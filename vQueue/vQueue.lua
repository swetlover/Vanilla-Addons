...
-- Sven Button Ende
		...
		vQueueFrame.title:SetText("vQueue(队伍集合石)")
		...
		vQueueFrame.topsectiontitle:SetText(L["<-- Select a catergory"])
							...
							if Wholefind(puncString, kkVal) > 0 then
								...
								if string.find(notifyForDungeon,kCat) then
									if (selectedRole == damageRole ) or (selectedRole == healerRole) or (selectedRole == tankRole) then
										...
										vQueueFrame.replyFrameMsg:SetText("Lvl " .. tostring(UnitLevel("player")) .. " " .. selectedRole .. " " .. vQueueDB.class)
...
function vQueue_addToWaitList(playerinfo)
	...
	if groups["waitlist"][args[1]] == nil then
		...
		newWaitEntry:SetFont("Fonts\\FRIZQT__.TTF", 10)
		...
		newWaitEntryLvl:SetFont("Fonts\\FRIZQT__.TTF", 10)
		...
		newWaitEntryClass:SetFont("Fonts\\FRIZQT__.TTF", 10)
		...
		newWaitEntryInvite:SetFont("Fonts\\FRIZQT__.TTF", 10)
...
function vQueue_addToGroup(category, groupinfo)
	...
	if groups[category][args[2]] == nil then
		...
		newHostEntryName:SetFont("Fonts\\FRIZQT__.TTF", 10)
		...
		newHostEntryLevel:SetFont("Fonts\\FRIZQT__.TTF", 10)
		...
		newHostEntrySize:SetFont("Fonts\\FRIZQT__.TTF", 10)
...
function vQueue_OnUpdate()
	...
	if whoRequestTimer > 2 then
		...
		if fixingChat then
			...
			DEFAULT_CHAT_FRAME:AddMessage(L["vQueue: So you need join channels manualy or in macro: 1 channel per macro"])
