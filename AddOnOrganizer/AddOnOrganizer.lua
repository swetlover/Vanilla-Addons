...
function CS_AddOnOrganizer_ListShowHide()
	...
	if (CS_AddOnOrganizer_List:IsVisible()) then
		...
		CS_AddOnOrganizer_List_Title:SetText ("插件管理器 Q猫汉化 v."..CS_ADDONORGANIZER_VERSIONNUMBER);
...
function CS_AddOnOrganizer_List_Update()
	...
	local i;
	CS_AddOnOrganizer_List_AddOnCount:SetText("插件: |CFFFFFFFF"..numaddons.."|r");
...
function CS_AddOnOrganizer_TitleButton_OnEnter()
	...
	if(loadondemand) then
		loadondemand = "|CFF00FF00是|r";
	else
		loadondemand = "|CFFFF0000否|r";
	end
	
	if(loadable ~= nil)then
		GameTooltip_AddNewbieTip(name, 1.0, 1.0, 1.0, title.."\n"..notes.."\n|CFFFFFFFF插件加载:|r|CFF00FF00是|r\n".."|CFFFFFFFF加载要求:|r "..loadondemand.."\n|CFFFFFFFF依赖:|r "..dependencies, 1);
	elseif(reason == "DISABLED") then
		GameTooltip_AddNewbieTip(name, 1.0, 1.0, 1.0, title.."\n"..notes.."\n|CFFFFFFFF插件加载:|r |CFFFF0000否|r\n|CFFFFFFFF原因:|r |CFFFF0000"..reason.."|r\n|CFFFFFFFF你可能仍然启用这插件.|r\n".."|CFFFFFFFF加载要求:|r "..loadondemand.."\n|CFFFFFFFF依赖:|r "..dependencies, 1);
	else
		GameTooltip_AddNewbieTip(name, 1.0, 1.0, 1.0, title.."\n"..notes.."\n|CFFFFFFFF插件加载:|r|CFFFF0000 否|r\n|CFFFFFFFF原因:|r |CFFFF0000"..reason.."|r\n".."|CFFFFFFFF加载要求:|r "..loadondemand.."\n|CFFFFFFFF依赖:|r "..dependencies, 1);
	end
end
