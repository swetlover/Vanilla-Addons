...
Default_SpamThrottle_KeywordFilterList = {
	"小白","淘宝","代购","店铺","钟水","Blessed Blade of the Windseeker", "item4game", "moneyforgames",
...
function UFOverHookEvents()
	if(time() - UFStartTime > 10 and UFInitialized == nil) then
		...
		SpamThrottleMessage(true,"聊天消息过滤屏蔽现在启用.");
    	UFStartTime = nil;
		UFInitialized = true;
		this:Hide();
...
function SpamThrottleCreateTooltip(STTooltip)
	if SpamThrottle_LastPlayerBanned ~= " " then STTooltip:AddLine("最后禁止的玩家: ".. SpamThrottle_LastPlayerBanned , 0, 1, 0); end
	if SpamThrottle_LastPlayerFiltered ~= " " then STTooltip:AddLine("最后过滤的玩家: ".. SpamThrottle_LastPlayerFiltered , 0, 1, 0); end
end
