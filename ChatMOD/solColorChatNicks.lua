...
-------------------------------------------------
	...
	if( not SCCN_HideChatButtons )  then SCCN_HideChatButtons = 1; end;
	...
	if( not SCCN_AutoGossipSkip ) then SCCN_AutoGossipSkip = 1; end;
	if( not SCCN_AutoDismount ) then SCCN_AutoDismount = 1; end;
	...
	SCCNOnScreenMessage = "";
	-- welcome dialog
	if( not SCCN_WELCOMESHOWED ) then SCCN_WELCOMESHOWED = "1.05b"; end;
...
function solColorChatNicks_PurgeDB()
	...
	if( SCCN_EntrysConverted > 0 ) then
		...
		SCCN_write("|CFF00FF00欢迎使用由LuaCN1.12整合修改的插件！");
	end	
	SCCN_write("|CFF00FF00更多插件请关注论坛 www.luacn.net\n |CFF00FF00论坛QQ群:71367796\n |CFF00FF00祝您游戏愉快！");
...
_G = getfenv(0)
     ...
     local tab = function(f)                         -- CHAT TAB
         if not f.styled then
			 ...
			 tab:SetPoint('LEFT', ChatFrameMenuButton, 'LEFT',25,0)
...
-----以下为聊天按钮框架的位置和输入框材质
	 ...
	 ChatFrameMenuButton:SetPoint('BOTTOMRIGHT', ChatFrame1, 'TOPLEFT', 27, 0)
	 ChatFrameMenuButton:SetScale(.8)

     local x = ({ChatFrameEditBox:GetRegions()})
     x[6]:SetVertexColor(.5, .5, .5) x[7]:SetVertexColor(.5, .5, .5) x[8]:SetVertexColor(.5, .5, .5)
...
-----以下为BottomButton按钮的位置
function SCCN_ChatFrame_OnLoad()

			-- Move the bottom scoll button to new spot
...
function SCCN_GOSSIP()
	...
	if SCCN_AutoGossipSkip == 1 then	
		...
		for i=1, getn(GossipOptions) do
			...
			if (GossipOptions[i] == "battlemaster" or GossipOptions[i] == "taxi" or GossipOptions[i] == "banker" or GossipOptions[i] == "The orb's markings match the brand on your hand." or GossipOptions[i] == "You see large cavernous tunnels") then
